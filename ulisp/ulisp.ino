  ...
  while (form != NULL) {
    ...
    for (int i = (sizeof(int) - 1) * 8; i >= 0; i = i - 8) {
      ...
      if (ch) {
        if (ch == '.') {
          p++;
          if (p > 3) error2("illegal IP address");
        }
...
EVAL:
  ...
  if ((uint32_t)StackBottom - (uint32_t)&stackpos > MAX_STACK) {
    Context = NIL;
...
#if defined(BOARD_HAS_PSRAM)
  if (!psramInit()) {
    Serial.print("the PSRAM couldn't be initialized");
    for (;;);
  }
  Workspace = (object*) ps_malloc(WORKSPACESIZE * 8);
  if (!Workspace) {
    Serial.print("the Workspace couldn't be allocated");
    for (;;);
  }
#endif
